...
export const ModelExportController: React.FC<ModelExportControllerProps> = ({
  ...
  const [activeMetricsTab, setActiveMetricsTab] = useState<'metrics' | 'modelcard' | 'config'>('metrics');

  const [bundleOptions, setBundleOptions] = useState<Required<ExportPackageOptions>>({
    includeTflite: true,
  ...
  useEffect(() => {
    API.listRuns()
      ...
      .catch((err) => console.error('Failed to list runs for export controller:', err));
  }, []);
  ...
  const calculateEstimatedSize = () => {
    ...
    const outputDim = 8;
    const rawWeightsSize = (hiddenDim * inputDim + hiddenDim + outputDim * hiddenDim + outputDim) * 4;
    ...
    if (bundleOptions.includeLogs) bytes += (currentRun?.logLines?.length || 10) * 80;
    ...
    <section className="rounded-2xl border border-[rgba(255,255,255,0.1)] bg-[#121211] p-5 shadow-xl transition-all">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#A3A3A0]">
              Last ned uavhengige modeller eller komplette bunter sammensatt med metrikker, modellkort og skript.
            </p>
          </div>
        </div>
              ...
              >
                {runs.map((r) => (
                  <option key={r.id} value={r.id} className="bg-[#1A1A1A] text-white">
                    {r.id.slice(0, 14)}... ({((r.finalMetrics?.accuracy || 0) * 100).toFixed(1)}%)
                  </option>
                ))}
              </select>
            </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#171716] p-3">
          <span className="text-[11px] font-medium text-[#888]">Nøyaktighet (Test)</span>
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-mono text-lg font-bold text-[#77F23B]">{accuracyPercent}%</span>
            <span className="text-[10px] text-[#A3A3A0]">val/test</span>
          </div>
        </div>

        <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#171716] p-3">
          <span className="text-[11px] font-medium text-[#888]">Tapsverdi (Loss)</span>
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-mono text-lg font-bold text-[#39D9E6]">{testLossVal}</span>
            <span className="text-[10px] text-[#A3A3A0]">cross-entropy</span>
          </div>
        </div>

        <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#171716] p-3">
          <span className="text-[11px] font-medium text-[#888]">Minnebruk (RAM / Flash)</span>
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-mono text-lg font-bold text-white">{memoryKb} KB</span>
            <span className="text-[10px] text-[#A3A3A0]">/ ~12 KB Flash</span>
          </div>
        </div>

        <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#171716] p-3">
          <span className="text-[11px] font-medium text-[#888]">Parametere & Inferens</span>
          <div className="mt-1 flex items-baseline gap-1.5">
            <span className="font-mono text-lg font-bold text-[#B25CFF]">{paramCount}</span>
        ...
        <div className="flex flex-col justify-between rounded-xl border border-[#39D9E6]/30 bg-[#161615] p-4.5 transition-all hover:border-[#39D9E6]/50">
          <div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="relative flex h-8 w-8 items-center justify-center rounded-lg bg-[#39D9E6]/10 text-[#39D9E6] border border-[#39D9E6]/20">
                  ...
                  <span className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-[#77F23B]" title="Klar (Ready)" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h4 className="text-xs font-bold text-white">TensorFlow Lite (.tflite)</h4>
                  ...
                  <span className="font-mono text-[10px] text-[#39D9E6]">FlatBuffers TFL3 • Float32</span>
                </div>
              </div>
            ...
            <p className="mt-2.5 text-xs text-[#A3A3A0] leading-relaxed">
              Standard binærformat for TensorFlow Lite Micro, Raspberry Pi, Python og Android. Inneholder opcodes for FullyConnected, ReLU og Softmax med innebygde etiketter.
            </p>

            <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-[#888]">
              <span className="flex items-center gap-1">
                <CheckCircle2 className="h-3 w-3 text-[#77F23B]" /> Norsk tokenisering
              </span>
              <span>•</span>
              <span className="flex items-center gap-1">
                <CheckCircle2 className="h-3 w-3 text-[#77F23B]" /> Null dynamisk heap
              </span>
            </div>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-[rgba(255,255,255,0.06)] space-y-2">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() =>
                  triggerDownload(API.getTfliteUrl(selectedRunId), 'tflite-standalone')
                }
                disabled={!selectedRunId || downloadingFormat === 'tflite-standalone'}
            ...
            <p className="text-center text-[10px] text-[#777]">
              Inkluderer .tflite, evaluation_metrics.json, MODEL_CARD.md og infer_tflite.py
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-between rounded-xl border border-[#8F2BFF]/30 bg-[#161615] p-4.5 transition-all hover:border-[#8F2BFF]/50">
          <div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    <h4 className="text-xs font-bold text-white">TensorFlow SavedModel</h4>
                    <span className="inline-flex items-center gap-1 rounded-full bg-[#77F23B]/10 px-1.5 py-0.2 text-[9px] font-semibold text-[#77F23B] border border-[#77F23B]/20">
                      <CheckCircle2 className="h-2.5 w-2.5" /> Klar
                    </span>
                  </div>
                  <span className="font-mono text-[10px] text-[#B25CFF]">TF 2.x • serving_default</span>
                </div>
              </div>
              <span className="rounded bg-[#8F2BFF]/20 px-2 py-0.5 font-mono text-[10px] font-bold text-[#B25CFF]">
                Server & Cloud
            ...
            <p className="mt-2.5 text-xs text-[#A3A3A0] leading-relaxed">
              Komplett TensorFlow modellkatalogstruktur med <code className="text-white">saved_model.pb</code>, variabler og assets (vokabularkart). Klar for direkte server-inferens og Docker TF Serving.
            </p>

            <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-[#888]">
              <span className="flex items-center gap-1">
                <CheckCircle2 className="h-3 w-3 text-[#77F23B]" /> REST / gRPC API
              </span>
              <span>•</span>
              <span className="flex items-center gap-1">
                <CheckCircle2 className="h-3 w-3 text-[#77F23B]" /> Batch inferens
              </span>
            </div>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-[rgba(255,255,255,0.06)] space-y-2">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() =>
                  triggerDownload(API.getSavedModelUrl(selectedRunId), 'savedmodel-standalone')
                }
                disabled={!selectedRunId || downloadingFormat === 'savedmodel-standalone'}
                className="flex-1 flex items-center justify-center gap-2 rounded-lg bg-[#222221] px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-[#2C2C2B] disabled:opacity-50"
              >
            ...
            <button
              ...
              disabled={!selectedRunId || downloadingFormat === 'savedmodel-bundle'}
            ...
            <p className="text-center text-[10px] text-[#777]">
              Inkluderer saved_model/, evaluation_report.md, training_config.json og load_saved_model.py
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex items-center justify-between rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#161615] p-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-mono text-xs font-bold text-white">Embedded C/C++ Header (.h)</span>
              <p className="text-[11px] text-[#888]">For Arduino Nano 33 BLE, ESP32, STM32</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <a
        ...
        <div className="flex items-center justify-between rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#161615] p-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-mono text-xs font-bold text-white">JSON Vektormatriser (.json)</span>
              <p className="text-[11px] text-[#888]">W1, b1, W2, b2 og ordforrådskart</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <a
          ...
          <div className="border-t border-[rgba(255,255,255,0.06)] p-4 space-y-4">
              ...
              <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
                ...
                <label className="flex items-center gap-2 rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#1A1A19] p-2.5 cursor-pointer text-xs">
                  <input
                    ...
                    className="rounded text-[#8F2BFF] focus:ring-0"
                  />
                  <span className="text-white">Treningskonfig & Tapskurver</span>
                </label>
                ...
                <label className="flex items-center gap-2 rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#1A1A19] p-2.5 cursor-pointer text-xs sm:col-span-2">
                  <input
                    ...
                    className="rounded text-[#8F2BFF] focus:ring-0"
                  />
                  <span className="text-white">Kjørbare testskript (Python inferens, Arduino skisse)</span>
                </label>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <span className="text-xs text-[#888]">
                Beregnet pakkestørrelse: <strong className="text-[#77F23B] font-mono">{formatBytes(calculateEstimatedSize())}</strong>
              </span>
              ...
              <button
                ...
                disabled={!selectedRunId || downloadingFormat === 'custom-package'}
            ...
            <div className="flex items-center justify-between border-b border-[rgba(255,255,255,0.06)] pb-2">
              ...
              <button
                onClick={() => {
                  const content =
                    activeMetricsTab === 'metrics'
                      ? JSON.stringify(evaluation || currentRun?.finalMetrics || {}, null, 2)
                      : activeMetricsTab === 'modelcard'
                      ? `# TinyML Model Card: ${currentRun?.projectId || 'ornith'}\nAccuracy: ${accuracyPercent}%\nParameters: ${paramCount}\nLanguage: Norwegian (nb-NO)`
                      : JSON.stringify(currentRun?.hyperparameters || {}, null, 2);
              ...
              {activeMetricsTab === 'metrics' && (
                JSON.stringify(
                  evaluation || {
                    ...
                    memoryKb: memoryKb,
                    evaluatedAt: currentRun?.completedAt || new Date().toISOString(),
                  },
                  null,
                  2
                )
