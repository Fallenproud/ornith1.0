...
export const DatasetMode: React.FC<DatasetModeProps> = ({
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'errors' | 'warnings' | 'valid'>('all');
  ...
  const handleRevalidateDataset = async (customRules?: DatasetValidationRule[]) => {
    if (!activeDataset) return;
  ...
  const filteredRecords = records.filter((r) => {
    ...
    if (onlyNorwegianChars && (!r.norwegianCharCount || r.norwegianCharCount.total === 0)) {
      return false;
    }
    ...
    if (statusFilter === 'warnings') {
      const hasWarn = r.validationFailures?.some((f) => f.severity === 'warning');
    ...
    if (selectedRuleFilter) {
      const failedThisRule = r.validationFailures?.some((f) => f.ruleId === selectedRuleFilter);
    ...
    <div className="flex h-full flex-col bg-[#0E0E0D] text-[#F4F4F2]">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[rgba(255,255,255,0.06)] bg-[#141414] px-6 py-4">
        <div>
          ...
          <p className="mt-1 text-xs text-[#A3A3A0]">
            Kilde: {meta?.source || 'Lokalt korpus'} • Lisens: {meta?.license || 'CC-BY-4.0'} •{' '}
            Opprettet: {meta?.createdAt ? new Date(meta.createdAt).toLocaleDateString('nb-NO') : 'Nå'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <div className="flex rounded-lg border border-[rgba(255,255,255,0.08)] bg-[#1A1A1A] p-1 text-xs">
            ...
            >
              ...
              <span>Valideringsregler & Sandkasse</span>
            </button>
          </div>
          ...
          >
            <RefreshCw className={`h-3.5 w-3.5 text-[#39D9E6] ${isRevalidating ? 'animate-spin' : ''}`} />
            <span>{isRevalidating ? 'Validerer...' : 'Kjør Validering på nytt'}</span>
          </button>
      ...
      {revalidationNotice && (
        <div className="flex items-center justify-between border-b border-[#39D9E6]/30 bg-[#39D9E6]/10 px-6 py-2.5 text-xs text-[#39D9E6]">
          ...
          <button onClick={() => setRevalidationNotice(null)} className="text-[#39D9E6] hover:opacity-80">
      ...
      {activeTab === 'rules' && (
        <div className="flex-1 overflow-hidden">
          <ValidationRulesManager
            ...
            onApplyRulesToActiveDataset={(newRules) => handleRevalidateDataset(newRules)}
          />
        </div>
      )}
          ...
          <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#141414] p-3.5">
              ...
              <div className="mt-2 flex items-baseline gap-1.5">
                ...
                <span className="text-[10px] text-[#77F23B]">Kvalitetssikret</span>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#141414] p-3.5">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="font-mono text-xl font-bold text-[#77F23B]">{validCount}</span>
                <span className="font-mono text-xs text-[#A3A3A0]">/ {totalCount}</span>
                {invalidCount > 0 && (
            ...
            <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#141414] p-3.5">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="font-mono text-xl font-bold text-[#FF9F0A]">{warningCount}</span>
                <span className="font-mono text-[10px] text-[#A3A3A0]">
            ...
            <div className="mb-6 rounded-xl border border-[#FF453A]/20 bg-[#171414] p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs font-semibold text-white">
                  ...
                  <span>Oppdagede Regelbrudd ({ruleFailures.length} regler med utslag)</span>
                </div>
                {selectedRuleFilter && (
                  <button
                  ...
                  >
                    <span>Nullstill regelfilter</span>
                    <X className="h-3 w-3" />
                  </button>
                )}
              </div>
              <p className="mt-1 text-[11px] text-[#A3A3A0]">
                Klikk på et regelkort for å filtrere tabellen til kun postene som feilet den regelen:
              </p>
                        ...
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`h-2 w-2 rounded-full ${
                              rf.severity === 'error' ? 'bg-[#FF453A]' : 'bg-[#FF9F0A]'
                            }`}
                          />
          ...
          <div className="mb-6 rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#141414] p-4">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {Object.entries(classDist).map(([cls, count]) => {
                ...
                const percent = ((countNum / (meta?.rowCount || 1)) * 100).toFixed(0);
                return (
              ...
              <div className="flex items-center justify-between bg-[#8F2BFF]/10 px-4 py-2 text-xs text-[#B25CFF] border-b border-[#8F2BFF]/20">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    Filtrerer på regel:{' '}
                    <strong>
                      {ruleFailures.find((r) => r.ruleId === selectedRuleFilter)?.ruleName ||
                        selectedRuleFilter}
                    </strong>
                  </span>
                </div>
              ...
              <table className="w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="sticky top-0 z-10 border-b border-[rgba(255,255,255,0.06)] bg-[#171716] font-mono text-[11px] text-[#A3A3A0]">
                    ...
                    <th className="py-2.5 px-3 font-medium">Norsk Tekst (Input)</th>
                    <th className="py-2.5 px-3 font-medium">Etikett</th>
                    <th className="py-2.5 px-3 font-medium">Validering & Regler</th>
                ...
                <tbody className="divide-y divide-[rgba(255,255,255,0.04)] font-sans">
                  {filteredRecords.length === 0 ? (
                    ...
                    filteredRecords.map((rec, i) => {
                      const isExpanded = expandedRecordId === (rec.id || String(i));
                      ...
                      return (
                        <React.Fragment key={rec.id || i}>
                          <tr
                            onClick={() =>
                              setExpandedRecordId(isExpanded ? null : (rec.id || String(i)))
                            }
                          ...
                          >
                            ...
                            <td className="py-2.5 px-3 font-medium text-white max-w-md break-words">
                              {rec.text || <span className="italic text-[#FF453A]">[Tom tekst]</span>}
                            </td>
                            ...
                            <td className="py-2.5 pr-4 text-right">
                              {rec.isValid ? (
                                ...
                                <span className="inline-flex items-center gap-1 text-[11px] text-[#FF453A]">
                                  <AlertCircle className="h-3 w-3" /> Feil ({failures.length})
                                </span>
                              )}
                            </td>
                          </tr>
                                ...
                                <div className="space-y-2 rounded-lg bg-[#1F1717] p-3 text-xs">
                                  <div className="flex items-center justify-between text-[#A3A3A0] border-b border-[rgba(255,255,255,0.06)] pb-1.5">
                                    <span className="font-bold text-white">
                                      Detaljert Valideringsrapport for Rad #{i + 1}
                                    </span>
                                    <span className="font-mono text-[10px]">
                                      {rec.norwegianCharCount?.total || 0} norske tegn (æ:
                                        ...
                                        <div className="flex-1">
                                          ...
                                          {f.actualValue && (
                                            <div className="mt-0.5 font-mono text-[10px] text-[#A3A3A0]">
                                              Detektert verdi:{' '}
                                              <span className="text-[#FF9F0A]">{f.actualValue}</span>
                                            </div>
                                          )}
                                        </div>
                                      </div>
