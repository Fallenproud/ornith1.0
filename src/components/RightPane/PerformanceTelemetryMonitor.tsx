...
export const PerformanceTelemetryMonitor: React.FC<{
  compact?: boolean;
  ...
  const [recentEntries, setRecentEntries] = useState<SnapshotTelemetryEntry[]>(
    getTelemetryHistory(12),
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-md bg-[#191918] border border-[rgba(255,255,255,0.06)] px-2.5 py-1 hover:border-[#39D9E6]/40 transition-colors cursor-pointer text-left"
            title="Klikk for å åpne ytelsestelemetri-dashboardet i RightPane"
          >
            <Clock className="h-3 w-3 text-[#A3A3A0]" />
            <span className="text-[11px] text-[#A3A3A0]">Snittlatens:</span>
            <span
              className={`font-mono text-xs font-bold ${getLatencyColor(metrics.avgDurationMs)}`}
            >
          ...
          <div className="hidden sm:flex items-center gap-1.5 rounded-md bg-[#191918] border border-[rgba(255,255,255,0.06)] px-2 py-1 font-mono text-[11px]">
            ...
            <span className="font-bold text-white">
              {metrics.currentReadsPerSec} dok/s
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-1.5 rounded-md bg-[#191918] border border-[rgba(255,255,255,0.06)] px-2 py-1 font-mono text-[11px]">
            ...
            <span className="font-bold text-[#77F23B]">
          ...
          {metrics.highConcurrencyEvents > 0 ? (
            <span className="flex items-center gap-1 rounded-full bg-[#FF9F0A]/15 border border-[#FF9F0A]/30 px-2 py-0.5 text-[10px] font-bold text-[#FF9F0A]">
              ...
              <span>
                {metrics.highConcurrencyEvents} Høykonkurranse-varsler
              </span>
            </span>
          ) : (
        ...
        <div className="flex items-center gap-3">
          ...
          <div
            className="flex items-end gap-1 h-5 px-1 bg-[#161615] rounded border border-[rgba(255,255,255,0.04)]"
            title="Siste onSnapshot-lesinger (latens i ms)"
          >
            {recentEntries.length > 0 ? (
              recentEntries.slice(-10).map((entry, idx) => {
                const heightPercent = Math.min(
                  100,
                  Math.max(20, (entry.processingDurationMs / 100) * 100),
                );
                return (
                  <div
                    key={entry.id || `entry-${entry.timestamp}-${idx}`}
              ...
              <div className="w-16 text-[9px] font-mono text-[#666] text-center">
                venter...
              </div>
            )}
          </div>
          ...
          {onOpenDashboard && (
            <button
              onClick={onOpenDashboard}
              className="flex items-center gap-1 rounded bg-[#1D1D1C] border border-[rgba(255,255,255,0.06)] px-2 py-1 text-[11px] font-medium text-[#39D9E6] hover:bg-[#252523] hover:text-white transition-colors"
              title="Åpne dedikert telemetri-dashboard"
            >
              <span>Dashboard</span>
          ...
          <button
            ...
            title={
              isExpanded
                ? "Skjul detaljert telemetri"
                : "Vis detaljert telemetri"
            }
          >
            {isExpanded ? (
      ...
      {isExpanded && (
        <div className="mt-2.5 pt-2.5 border-t border-[rgba(255,255,255,0.05)] grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px]">
          ...
          <div className="bg-[#161615] p-2 rounded-lg border border-[rgba(255,255,255,0.04)]">
            ...
            <div className="font-mono text-sm font-bold text-[#39D9E6] mt-0.5">
              {metrics.totalDocsRead}
            </div>
          </div>
          <div className="bg-[#161615] p-2 rounded-lg border border-[rgba(255,255,255,0.04)]">
