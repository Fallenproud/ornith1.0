...
const MOJIBAKE_PATTERNS = [
  ...
  {
    pattern: /â€“|â€”|â€™|â€œ|â€/g,
    display: "â€ (ødelagt anførselstegn/tankestrek)",
  },
];
...
export function detectMojibake(text: string): {
  found: boolean;
  samples: string[];
} {
...
export const DEFAULT_NORWEGIAN_VALIDATION_RULES: DatasetValidationRule[] = [
  {
    ...
    description:
      "Sikrer at tekstfeltet ikke er tomt eller utelukkende består av mellomrom.",
    type: "missing_values",
    enabled: true,
    severity: "error",
    params: {
      ...
      maxMissingPercent: 0,
      customErrorMessage:
        "Tekstfeltet kan ikke være tomt eller kun bestå av mellomrom.",
    },
  },
  {
    ...
    description:
      "Sikrer at hver rad har en definert klasse/etikett for veiledet trening.",
    type: "missing_values",
    enabled: true,
    severity: "error",
    params: {
  ...
  },
  {
    ...
    description:
      "Kontrollerer at innholdet i tekstkolonnen er en gyldig streng.",
    type: "column_type",
    enabled: true,
    severity: "error",
    params: {
  ...
  },
  {
    ...
    description:
      "Sikrer at setninger er tilpasset RAM- og bufferstørrelse på mikrokontroller (3 - 220 tegn).",
    type: "text_length",
    enabled: true,
    severity: "error",
    params: {
      ...
      maxLength: 220,
      customErrorMessage:
        "Tekstlengden må være mellom 3 og 220 tegn for kantlagring.",
    },
  },
  {
    id: "rule-word-count",
    ...
    params: {
      ...
      maxWords: 35,
      customErrorMessage:
        "Ordtellingen faller utenfor forventet område (1 - 35 ord).",
    },
  },
  {
    id: "rule-range-limits",
    name: "Område- og grensebegrensninger",
    description:
      "Styrer terskelverdier for tegnlengde, ordtelling og token-buffer tilpasset TinyML.",
    type: "range_limits",
    enabled: true,
    severity: "error",
    params: {
      ...
      maxTokens: 64,
      customErrorMessage:
        "Ytringen overskrider tillatte terskelverdier for lengde, ord eller token-ramme.",
    },
  },
  {
    id: "rule-norwegian-frequency",
    name: "Norsk tegnfrekvens og særtegn",
    description:
      "Krever minimum prosentandel og antall av de særnorske tegnene æ, ø og å.",
    ...
    params: {
      ...
      customErrorMessage:
        "Teksten tilfredsstiller ikke kravet til norsk tegnfrekvens eller særnorske tegn.",
    },
  },
  {
    ...
    description:
      "Oppdager og flagger ødelagt tegnkoding i norske tegn (f.eks. Ã¦, Ã¸, Ã¥).",
    type: "ban_mojibake",
    enabled: true,
    severity: "error",
    params: {
      targetColumn: "text",
      customErrorMessage:
        "Inneholder ødelagt UTF-8-tegnkoding (mojibake) for norske bokstaver.",
    },
  },
  {
    ...
    description:
      "Overvåker tilstedeværelse av særnorske tegn for å sikre reell norsk språktrening.",
    ...
    params: {
      ...
      minNorwegianChars: 1,
      customErrorMessage:
        "Teksten inneholder ingen av de særnorske tegnene æ, ø eller å.",
    },
  },
  {
    ...
    description:
      "Varsler dersom det oppdages formblanding (f.eks. Nynorsk-ord i Bokmål-datasett).",
    ...
    params: {
      ...
      dialectTarget: "Bokmål",
      customErrorMessage:
        "Ytringen inneholder ordformer som avviker fra den valgte språkformen.",
    },
  },
  {
    id: "rule-unique-text",
    ...
    description:
      "Oppdager identiske ytringer for å hindre overtilpasning i treningssettet.",
    type: "unique_text",
    enabled: true,
    severity: "warning",
    params: {
...
export function validateSingleRecord(
  ...
  const text =
    typeof record.text === "string" ? record.text : String(record.text || "");
  const label =
  ...
  for (const rule of rules) {
    ...
    switch (rule.type) {
      case "missing_values": {
        ...
        if (rule.params.allowNull === false && targetVal === null) {
          failures.push({
            ruleId: rule.id,
            ruleName: rule.name,
            ruleType: rule.type,
            message:
              rule.params.customErrorMessage ||
              `Feltet '${targetCol}' kan ikke være null.`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: "null",
          });
        } else if (rule.params.disallowEmpty && isEmpty) {
          failures.push({
            ruleId: rule.id,
            ruleName: rule.name,
            ruleType: rule.type,
            message:
              rule.params.customErrorMessage ||
              `Feltet '${targetCol}' mangler eller er tomt.`,
            severity: rule.severity,
            targetColumn: targetCol,
          ...
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Feltet '${targetCol}' inneholder kun tomrom.`,
            severity: rule.severity,
            targetColumn: targetCol,
      ...
      case "column_type": {
        ...
        if (actualType !== expected) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Forventet datatype '${expected}' i kolonne '${targetCol}', men fant '${actualType}'.`,
            severity: rule.severity,
            targetColumn: targetCol,
      ...
      case "text_length": {
        ...
        if (len < min) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Tekstlengde (${len} tegn) er kortere enn påkrevd minimum (${min} tegn).`,
            severity: rule.severity,
            targetColumn: targetCol,
          ...
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Tekstlengde (${len} tegn) overskrider tillatt maksimum (${max} tegn).`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: `${len} tegn`,
          });
      ...
      case "word_count": {
        ...
        if (count < minW) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Ordtelling (${count} ord) er under minimum på ${minW} ord.`,
            severity: rule.severity,
            targetColumn: targetCol,
          ...
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Ordtelling (${count} ord) er over tillatt grense på ${maxW} ord.`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: `${count} ord`,
          });
      ...
      case "range_limits": {
        ...
        if (rule.params.minLength != null && charLen < rule.params.minLength) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Tegnlengde (${charLen}) er under nedre grense på ${rule.params.minLength} tegn.`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: `${charLen} tegn`,
          });
        } else if (
          rule.params.maxLength != null &&
          charLen > rule.params.maxLength
        ) {
          failures.push({
            ruleId: rule.id,
            ruleName: rule.name,
            ...
            message:
              rule.params.customErrorMessage ||
              `Tegnlengde (${charLen}) overskrider øvre grense på ${rule.params.maxLength} tegn.`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: `${charLen} tegn`,
          });
        ...
        if (rule.params.minWords != null && wordCount < rule.params.minWords) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Antall ord (${wordCount}) er under grensen på ${rule.params.minWords} ord.`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: `${wordCount} ord`,
          });
        } else if (
          rule.params.maxWords != null &&
          wordCount > rule.params.maxWords
        ) {
          failures.push({
            ruleId: rule.id,
            ruleName: rule.name,
            ...
            message:
              rule.params.customErrorMessage ||
              `Antall ord (${wordCount}) overskrider tillatt maksimum på ${rule.params.maxWords} ord.`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: `${wordCount} ord`,
          });
        ...
        if (rule.params.maxTokens != null && tokenEst > rule.params.maxTokens) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Estimert token-forbruk (~${tokenEst}) overskrider mikrokontroller-grensen på ${rule.params.maxTokens} tokens.`,
            severity: rule.severity,
            targetColumn: targetCol,
        ...
        // Numerical value boundaries if target is a number
        if (
          ...
          if (rule.params.minValue != null && numVal < rule.params.minValue) {
            failures.push({
              ruleId: rule.id,
              ruleName: rule.name,
              ruleType: rule.type,
              message:
                rule.params.customErrorMessage ||
                `Numerisk verdi (${numVal}) er under minimumsgrensen på ${rule.params.minValue}.`,
              severity: rule.severity,
              targetColumn: targetCol,
              actualValue: numVal,
            });
          } else if (
            rule.params.maxValue != null &&
            numVal > rule.params.maxValue
          ) {
            failures.push({
              ruleId: rule.id,
              ruleName: rule.name,
              ...
              message:
                rule.params.customErrorMessage ||
                `Numerisk verdi (${numVal}) overskrider maksimumsgrensen på ${rule.params.maxValue}.`,
              severity: rule.severity,
              targetColumn: targetCol,
              actualValue: numVal,
            });
      ...
      case "norwegian_char_frequency": {
        ...
        const actualFrequencyPct = parseFloat(
          ((norwegianChars / cleanLen) * 100).toFixed(2),
        );
        ...
        if (norwegianChars < minChars) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Mangler påkrevde særnorske tegn (fant ${norwegianChars}, krever minst ${minChars} av æ, ø, å).`,
            severity: rule.severity,
            targetColumn: targetCol,
          ...
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Frekvens av norske tegn (${actualFrequencyPct}%) er under terskelen på ${minFreq}%.`,
            severity: rule.severity,
            targetColumn: targetCol,
        ...
        if (
          ...
          rule.params.requiredNorwegianCharacters.length > 0
        ) {
          const missingReq = rule.params.requiredNorwegianCharacters.filter(
            (c) => !strVal.includes(c),
          );
          if (missingReq.length > 0) {
            failures.push({
              ...
              message:
                rule.params.customErrorMessage ||
                `Mangler spesifiserte særnorske tegn: [${missingReq.join(", ")}].`,
              severity: rule.severity,
              targetColumn: targetCol,
      ...
      case "norwegian_char_presence": {
        ...
        if (norwegianCharCount.total < minNeeded) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Teksten mangler særnorske tegn (fant ${norwegianCharCount.total}, krevde ${minNeeded}).`,
            severity: rule.severity,
            targetColumn: targetCol,
      ...
      case "unique_text": {
        ...
        if (occurrences > 1) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Ytringen forekommer ${occurrences} ganger i datasettet.`,
            severity: rule.severity,
            targetColumn: targetCol,
      ...
      case "label_whitelist": {
        ...
        if (allowed.length > 0 && !allowed.includes(label)) {
          failures.push({
            ...
            message:
              rule.params.customErrorMessage ||
              `Klassen '${label}' er ikke i listen over tillatte klasser: [${allowed.join(", ")}].`,
            severity: rule.severity,
            targetColumn: targetCol,
            actualValue: label,
          });
      ...
      case "regex_match": {
        if (rule.params.regexPattern) {
          try {
            const re = new RegExp(
              rule.params.regexPattern,
              rule.params.regexFlags || "i",
            );
            const strVal = String(targetVal || "");
            if (!re.test(strVal)) {
              failures.push({
                ...
                message:
                  rule.params.customErrorMessage ||
                  `Teksten matcher ikke det regulære uttrykket /${rule.params.regexPattern}/.`,
                severity: rule.severity,
                targetColumn: targetCol,
...
export function validateDatasetRecords(
  ...
  const norwegianScore =
    totalNorwegianChars > 0
      ? Math.min(1.0, totalNorwegianChars / Math.max(records.length * 0.45, 1))
      : 0.85;

  const ruleFailures = Array.from(ruleFailuresMap.values()).filter(
    (s) => s.failedCount > 0,
  );
  ...
  switch (type) {
    case "missing_values":
      return {
        ...
        category: "Struktur",
        description:
          "Forhindrer tomme, null- eller kun-mellomrom verdier i kritiske kolonner.",
    ...
    case "column_type":
      return {
        ...
        category: "Skjema",
        description:
          "Bekrefter at kolonneverdien har forventet datatype (f.eks. tekststreng).",
        defaultSeverity: "error",
      };
    case "text_length":
      return {
        ...
        category: "Grenser",
        description:
          "Sikrer at setninger holder seg innenfor minimum og maksimum tegnantall.",
        defaultSeverity: "error",
      };
    case "word_count":
      return {
    ...
    case "range_limits":
      return {
        ...
        category: "Grenser",
        description:
          "Definerer absolutte min/maks grenser for tegn, ord, tokens og tallverdier.",
        defaultSeverity: "error",
      };
    case "norwegian_char_frequency":
      return {
        ...
        category: "Norsk NLP",
        description:
          "Krever en minimumsprosent eller minimum antall særnorske tegn (æ, ø, å) i teksten.",
        defaultSeverity: "warning",
      };
    case "unique_text":
      return {
        ...
        category: "Kvalitet",
        description:
          "Varsler eller avviser identiske tekstytringer i korpuset.",
        defaultSeverity: "warning",
      };
    case "label_whitelist":
      return {
        ...
        category: "Skjema",
        description:
          "Tillater kun godkjente etiketter/klasser fra en forhåndsdefinert liste.",
        defaultSeverity: "error",
      };
    case "norwegian_char_presence":
      return {
        ...
        category: "Norsk NLP",
        description:
          "Overvåker eller krever at ytringer inneholder særnorske tegn.",
        defaultSeverity: "warning",
      };
    case "norwegian_dialect":
      return {
        ...
        category: "Norsk NLP",
        description:
          "Gjennomsøker for uoverensstemmelser mellom Bokmål og Nynorsk.",
        defaultSeverity: "warning",
      };
    case "ban_mojibake":
      return {
        ...
        category: "Norsk NLP",
        description:
          "Oppdager feilkodede tegn som Ã¦, Ã¸, Ã¥ og Â fra feil UTF-8/Latin-1 parsing.",
        defaultSeverity: "error",
      };
    case "regex_match":
      return {
        ...
        category: "Avansert",
        description:
          "Validerer tekst eller etikett mot et tilpasset regex-mønster.",
        defaultSeverity: "error",
      };
  }
}
